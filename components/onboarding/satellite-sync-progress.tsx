...
export function SatelliteSyncProgress({ poll = true }: { poll?: boolean }) {
  ...
  if (job.status === 'failed') {
    return (
      <p className="rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-sm text-destructive">
        Error en sincronización satelital: {job.error ?? 'reintentar más tarde'}
      </p>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-primary/30 bg-primary/5 px-4 py-4 space-y-3">
      <p className="flex items-center gap-2 text-sm font-medium">
        ...
        Sincronización satelital Copernicus
      </p>
      <Progress value={job.percent} className="h-2" />
      <p className="text-xs text-muted-foreground">
        {job.zonesDone} de {job.zonesTotal} zonas ({job.percent}%)
      </p>
    </div>
  );
}
